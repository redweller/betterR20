...
var DungeonAlchemistImporter = (() => {
  ...
  const createObj = (objType, obj, ...others) => {
    if (objType === "door" || objType === "window") {
      const conf = DungeonAlchemistImporter[`config${objType.toSentenceCase()}s`];
  ...
  const createWall = (page, map, wall, originalGridSize, gridSize, version) => {
    // BEGIN MOD
    if ((wall.type === 1 && DungeonAlchemistImporter.configDoors === "LS")
        || (wall.type === 2 && DungeonAlchemistImporter.configWindows === "LS")) {
      wall.type = 0;
    } else if ((wall.type === 1 && DungeonAlchemistImporter.configDoors === "LT")
              || (wall.type === 2 && DungeonAlchemistImporter.configWindows === "LT")) {
      wall.type = 4;
    }
    // END MOD
  ...
  const resizeMap = (gridSize, grid, page, map) => {
    // BEGIN MOD
    if (!DungeonAlchemistImporter.configResize) return;
    //END MOD
  ...
  return {
    ...
    process: ($editor, txt) => {
      ...
      DungeonAlchemistImporter.configLights = $editor.find(".lights-config").val();
      DungeonAlchemistImporter.configResize = $editor.find(".resize-config").prop("checked");
      handleInput(msg);
      $editor.off();
    ...
    dialog: `
      ...
      <input class="vtt-export" style="display:none" type="file" name="file" accept=".txt"/>
      </div>
    `,
  }
})();
