...
globalThis.Renderer = function () {
	...
	this._renderTable = function (entry, textStack, meta, options) {
		...
		autoRollMode ? RollerUtil.getFullRollCol(headerRowMetas.last()[0]) : null;
			...
			case "@recharge": {
				...
				switch (tag) {
					...
					case "@recharge": {
						...
						displayText ? displayText.split("") : null; // "m" for "minimal" = no brackets
...
Renderer.hover = class {
	...
	static getShowWindow ($content, position, opts) {
		...
		if (opts.pageUrl && !position.window._IS_POPOUT && !Renderer.get().isInternalLinksDisabled()) {
			$(`<a class="hwin__top-border-icon glyphicon glyphicon-modal-window" title="Go to Page" href="${opts.pageUrl}"></a>`)
				.appendTo($brdTopRhs);
		}

		if (!position.window._IS_POPOUT && !opts.isPopout) {
			$(`<span class="hwin__top-border-icon glyphicon glyphicon-new-window hvr__popout" title="Open as Popup Window"></span>`)
				.on("click", evt => {
					evt.stopPropagation();
		...
		$(`<span class="hwin__top-border-icon glyphicon glyphicon-remove" title="Close (CTRL to Close All)"></span>`)
			.on("click", (evt) => {
				evt.stopPropagation();
