...
function Renderer () {
	...
	this._renderString_renderTag = function (textStack, meta, options, tag, text) {
		// region Plugins
		// Generic
		for (const plugin of this._getPlugins("string", "tag")) {
		...
		switch (tag) {
			...
			default: {
				const {name, source, displayText, others, page, hash, hashPreEncoded, pageHover, hashHover, hashPreEncodedHover, preloadId, linkText} = Renderer.utils.getTagMeta(tag, text);
	...
	getTagEntry (tag, text) {
		switch (tag) {
			...
			case "@savingThrow": { // format: {@savingThrow str 5} or {@savingThrow str 5|Display Text} or {@savingThrow str 5|Display Text|Roll Name Text}
				...
				const [abil, rawScoreOrMod] = abilAndScoreOrScore.split(" ").map(it => it.trim().toLowerCase()).filter(Boolean);
			...
			// format: {@skillCheck animal handling 5} or {@skillCheck animal handling 5|Display Text}
			//   or {@skillCheck animal handling 5|Display Text|Roll Name Text}
			case "@skillCheck": {
				const fauxEntry = {
					type: "dice",
					rollable: true,
				...
				const parts = skillAndMod.split(" ").map(it => it.trim().toLowerCase()).filter(Boolean);
				const bonusPart = parts.pop();
				const bonus = Number(bonusPart) || 0;
				const mod = UiUtil.intToBonus(bonus);
				const skill = parts.join(" ");
...
Renderer.monster = {
	...
	getRenderWithPlugins ({renderer, mon, fn}) {
		return renderer.withPlugin({
			...
			pluginType: "*",
			fnPlugin: () => {
				if (mon._summonedBySpell_levelBase == null) return null;
				return {
					additionalData: {
	...
	_getCompactRenderedString (mon, renderer, opts) {
		...
		} else if (isCr && ScaleSummonCreature.isCrInScaleRange(mon)) {
	...
	getSkillsString (renderer, mon) {
		function doSortMapJoinSkillKeys (obj, keys, joinWithOr) {
			const toJoin = keys.sort(SortUtil.ascSort).map(s => `<span data-mon-skill="${s.toTitleCase()}|${obj[s]}">${renderer.render(`{@skill ${s.toTitleCase()}}`)} ${Renderer.get().render(`{@skillCheck ${s} ${obj[s]}}`)}</span>`);
