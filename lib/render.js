	...
	this._renderImage = function (entry, textStack, meta, options) {
		...
		if (entry.title || entry.mapRegions) {
			textStack[0] += `<div class="rd__image-title">
				...
				${entry.mapRegions ? `<button class="btn btn-xs btn-default rd__image-btn-viewer" onclick="RenderMap.showViewer(event, this)" data-rd-packed-map="${this._renderImage_getMapRegionData(entry)}"><span class="glyphicon glyphicon-picture"></span> Dynamic Viewer</button>` : ""}
			</div>`;
...
Renderer.hover = {
	...
	getShowWindow ($content, position, opts, sourceData) {
		...
		function handleDragClick () {
			// $hov.css("z-index", ""); // remove the temporary z-boost... // TODO set cur max
			// FIXME
